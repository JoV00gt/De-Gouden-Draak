...
<div class="container w-3/4 py-3 mx-auto px-4">
    <h1 class="text-2xl font-bold mb-4">{{__('dishes-edit-title')}}</h1>
    <form action="{{ route('menu.update', $dish->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select id="category" name="category" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                <option value="">{{__('dishes-select-default')}}</option>
        ...
        <div class="flex items-center justify-end">
            <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">{{__('dishes-edit')}}</button>
        </div>
    </form>
</div>
</x-checkout-layout>